...
- Forced Long Words Wrap: Forces words that exceed the character limit to be broken across multiple lines.
- Automatic fixes of common mapping mistakes (e.g. replaces "?" with "？")
- Automatic Backups: The original file is safely preserved and renamed with a `.bac.xlsx` extension before saving the new changes.
...
6. A success popup will tell you how many cells were modified. Your new file will have the exact same name as the original, and your original file will now end in `.bac.xlsx`.
